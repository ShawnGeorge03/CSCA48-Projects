...
void load_ingredients(void)
{
  ...
  for(int i = 0; i < MAT_SIZE; i++) printf("%d     ",i );
  printf("\n");
  for (int i=0; i<MAT_SIZE; i++){
    printf("%d ", i);
    for (int j=0; j<MAT_SIZE; j++) printf("%2.2lf, ",AdjMat[i][j]);
    printf("\n");
  }
...
int ingredient_index(char source_ingredient[MAX_STR_LEN])
{
     ...
     *****/
     for(int i = 0; i < MAT_SIZE; i++)
       if(strcmp(source_ingredient, ingredients[i]) == 0) return i;
     return -1;
}
...
void related_ingredients(char source_ingredient[MAX_STR_LEN])
{
     ...
     * ********/
     ...
     if(ingredient_index(source_ingredient) == -1) return;
...
intNode *related_k_dist(intNode *h, char source_ingredient[MAX_STR_LEN], int k, int dist)
{
     ...
     *******/
     ...
     for(int col = 0; col < MAT_SIZE; col++){
       if(AdjMat[index][col] > 0){
         if(searchInt(h, col) == 0) h = insertInt(h, col);
         h = related_k_dist(h, ingredients[col], k, dist + 1);
       }
     }
...
intNode *related_with_restrictions(char source_ingredient[MAX_STR_LEN], char avoid[MAX_STR_LEN], int k_source, int k_avoid)
{
     ...
     *****/
     ...
     intNode *src = related_k_dist(NULL, source_ingredient, k_source, 0);
     intNode *avd = related_k_dist(NULL, avoid, k_avoid, 0);

     intNode *h = NULL;
     intNode *curr = src;
     while(curr != NULL){
       if(searchInt(avd, curr->x) == 0 && curr->x != ingredient_index(avoid))
         h = insertInt(h, curr->x);
       curr = curr->next;
     }
...
void substitute_ingredient(char recipe[10][MAX_STR_LEN], char to_change[MAX_STR_LEN])
{
   ...
   */
    ...
    ******/
    ...
    if(ingredient_index(to_change) == -1) return;

    int max = 0, max_pos = 0, replace_pos = 0;

    int ingredients_list[MAT_SIZE] = {0};

    for(int row = 0; strcmp(recipe[row], "") != 0; row++){
      //Finds the pos of the ingredient to be replaced
      if(strcmp(recipe[row], to_change) == 0){
        replace_pos = row;
        //No need to check its column in the AdjMat
        continue;
      }

      //Sums up the columns of the recipe items
      for(int col = 0; col < MAT_SIZE; col++)
        ingredients_list[col] += AdjMat[ingredient_index(recipe[row])][col];
    }

    //Removes ingredients from the array that appear in the recipe
    for(int i = 0; i < MAT_SIZE; i++){
      for(int j = 0; strcmp(recipe[j], "") != 0; j++){
        if(strcmp(ingredients[i], recipe[j]) == 0) ingredients_list[i] = 0;
      }
    }

    //Finds max in the ingredients_list
    for(int i = 0; i < MAT_SIZE; i++){
      if(ingredients_list[i] >= max){
        max = ingredients_list[i];
        max_pos = i;
      }
    }

    if(max == 0){
      strcpy(recipe[replace_pos], "");
    } else{
      strcpy(recipe[replace_pos], ingredients[max_pos]);
    }
