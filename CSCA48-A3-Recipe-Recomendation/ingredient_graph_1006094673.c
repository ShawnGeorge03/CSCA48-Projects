...
int ingredient_index(char source_ingredient[MAX_STR_LEN])
{
     ...
     *****/
     for(int row = 0; row < MAT_SIZE; row++)
       if(strcmp(source_ingredient, ingredients[row]) == 0) return row;
     return -1;
}
...
void related_ingredients(char source_ingredient[MAX_STR_LEN])
{
     ...
     * ********/
     ...
     for(int col = 0; col < MAT_SIZE; col++){
       if(AdjMat[ingredient_index(source_ingredient)][col] > 0){
         printf("%s\n",ingredients[col]);
       }
     }
}
...
intNode *related_k_dist(intNode *h, char source_ingredient[MAX_STR_LEN], int k, int dist)
{
     ...
     *******/
     ...
     if(dist <= k){
       ...
       if(index == -1) return h;
       for(int row = 0; row < MAT_SIZE; row++){
         if(AdjMat[row][index] > 0){
           if(searchInt(h, row) == 0) h = insertInt(h, row);
...
intNode *related_with_restrictions(char source_ingredient[MAX_STR_LEN], char avoid[MAX_STR_LEN], int k_source, int k_avoid)
{
     ...
     *****/
     ...
     if(ingredient_index(source_ingredient) == -1) return NULL;
     if(k_source < 0 || k_avoid < 0) return NULL;
...
void substitute_ingredient(char recipe[10][MAX_STR_LEN], char to_change[MAX_STR_LEN])
{
   ...
   */
    ...
    //Removes ingredient that exist in recipe
    for(int row = 0; row < MAT_SIZE; row++){
      for(int col = 0; strcmp(recipe[col], "") != 0; col++){
        if(strcmp(ingredients[row], recipe[col]) == 0){
          ingredients_list[row] = -1;
        }
      }
    }
