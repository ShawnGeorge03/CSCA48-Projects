...
{
  BST_Node *temp;
  temp = (BST_Node *)calloc(1,sizeof(BST_Node));
  if(temp != NULL)
  {
      temp = node;
...
BST_Node *find_successor(BST_Node *right_child_node)
{
    ...
    if(right_child_node->right != NULL)
    {
      return Min_Node(right_child_node->right);
    }
    else
    {
      return NULL;
...
void BST_makePlayList(BST_Node *root)
{
     ...
     ****/
     ...
     if(temp != NULL)
     {
       BST_makePlayList(temp->left);
       playlist_head = playlist_insert(playlist_head,temp->freq,temp->bar,temp->index);
       BST_makePlayList(temp->right);
     }
...
void BST_inOrder(BST_Node *root, int depth)
{
     ...
     ****/
     ...
     if(temp != NULL)
     {
       BST_inOrder(temp->left,depth);
       depth++;
       printf("Depth=%d, Bar:Index (%d:%f), F=%f Hz\n",\
       depth,temp->bar, temp->index, temp->freq);
       BST_inOrder(temp->right,depth);
       depth++;
     }
...
void BST_preOrder(BST_Node *root, int depth)
{
     ...
     ****/
     ...
     if(temp != NULL)
     {
       printf("Depth=%d, Bar:Index (%d:%f), F=%f Hz\n",\
       depth,temp->bar, temp->index, temp->freq);
       BST_preOrder(temp->left,depth);
       depth++;
       BST_preOrder(temp->right,depth);
       depth++;
     }
}
...
void BST_postOrder(BST_Node *root,int depth)
{
     ...
     ****/
     ...
     if(temp != NULL)
     {
       BST_postOrder(temp->left,depth);
       depth++;
       BST_postOrder(temp->right,depth);
       depth++;
       printf("Depth=%d, Bar:Index (%d:%f), F=%f Hz\n",\
       depth,temp->bar, temp->index, temp->freq);
     }
...
void delete_BST(BST_Node *root)
{
     ...
     ****/
     ...
     delete_BST(temp->left);
     delete_BST(temp->right);
     free(temp);
}
...
void BST_shiftFreq(BST_Node *root, char note_src[5], char note_dst[5])
{
     ...
     ****/
      ...
      while(note_src[i]!='\0')
      {
        if(strcmp(note_src, note_names[i]) == 0 && strcmp(note_dst, note_names[i]) == 0)
        {
          if(root == NULL)
          {
            return NULL;
          ...
          if(root->freq < note_freq[i])
          {
              return BST_shiftFreq(root->right, note_src, note_dst);
          }
          else
          {
            return BST_shiftFreq(root->left, note_src, note_dst);
          }
        }
      }
}
...
BST_Node *BST_harmonize(BST_Node *root, int semitones, double time_shift)
{
     ...
     ****/
     ...
     i = 0;
     while(root->freq != note_freq[i])
     {
          i++;
     }
     if((i + semitones < 100) && (i + semitones > -1) && ((root->index + time_shift) < 1) && (root->index + time_shift) > -1)
     {
          new_node = newBST_Node(note_freq[i + semitones], root->bar, (time_shift) + root->index);
          if(BST_search(root, root->bar, root->index) != NULL)
          {
            new_node = newBST_Node(note_freq[i + semitones],root->bar,(root->index + time_shift + 0.00000001));
          }
